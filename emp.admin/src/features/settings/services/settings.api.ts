...
const mapSettings = (row: ApiRecord): ShopSettings => ({
  ...
  defaultShippingFee: toNumber(row.defaultShippingFee ?? row.default_shipping_fee),
  freeShippingMinSubtotal: toNumber(row.freeShippingMinSubtotal ?? row.free_shipping_min_subtotal),
...
export const settingsApi = {
  get: async (): Promise<ShopSettings> => {
    const data = await unwrap(http.get<ApiResponse<ApiRecord>>('/admin/settings'))
    return mapSettings(data ?? {})
  },

  update: async (payload: ShopSettings): Promise<ShopSettings> => {
    const data = await unwrap(http.patch<ApiResponse<ApiRecord>>('/admin/settings', payload))
    return mapSettings(data ?? {})
  },
}
